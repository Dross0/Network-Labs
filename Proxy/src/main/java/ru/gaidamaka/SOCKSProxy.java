...
import ru.gaidamaka.attachment.AcceptHandler;
import ru.gaidamaka.attachment.Attachment;
import ru.gaidamaka.attachment.ClientHandler;
import ru.gaidamaka.attachment.ServerHandler;
import ru.gaidamaka.dns.DNSQueueOperationsWithExceptionsStrategy;
...
public class SOCKSProxy implements Closeable {
    ...
    private void handleAttachment(@NotNull Object attachmentObj) {
        ...
        switch (attachment.getType()) {
            case ACCEPT_HANDLER:
                handle((AcceptHandler) attachment);
                break;
            case DNS_RESOLVER:
                handle((DNSResolver) attachment);
                break;
            case CLIENT_HANDLER:
                handle((ClientHandler) attachment);
                break;
            case SERVER_HANDLER:
                handle((ServerHandler) attachment);
                break;
            default:
